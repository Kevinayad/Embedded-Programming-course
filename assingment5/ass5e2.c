...
/* CODE IS MOSTLY REUSED FROM WORKPACKAGE 4 exercise 1 */
...
pinMode(5, OUTPUT); // Sets the pin to output
pinMode(6, OUTPUT); // Sets the pin to output
...
if(celsius>0&&celsius<10){//if temperature lower than 10 but higher than 1
    ...
    digitalWrite(3, LOW);//LED 4 off
    digitalWrite(2, LOW);//LED 5 off
...
if(celsius>31&&celsius<40){//if temperature lower than 40 but higher than 31
    ...
    digitalWrite(5, HIGH);//LED 2 on
    digitalWrite(4, HIGH);//LED 3 on
    digitalWrite(3, HIGH);//LED 4 on
    digitalWrite(2, LOW);//LED 5 of
}
